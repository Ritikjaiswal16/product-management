...
const Inventory = () => {
    ...
    const handleSave = async (requestBody) => {
        try {
            ...
            if(Object.keys(showAddInventoryModal).length){
                response = (await axios.put(`http://192.168.1.13:8000/api/inventory/${showAddInventoryModal.inventory_id}`, requestBody, getHeaderOptions(token))).data;
                setToastInfo({type:"success", message:"Inventory updated successfully."});
            } else{
                response = (await axios.post('http://192.168.1.13:8000/api/inventory', requestBody, getHeaderOptions(token))).data;
    ...
    const handleDeleteInventory = async (inventoryId) => {
        try{
            setIsLoading(true);
            await axios.delete(`http://192.168.1.13:8000/api/inventory/${inventoryId}`, getHeaderOptions(token));
            setPagination(1);
            setShowDeleteModal(null);
            setToastInfo({type:"success", message:"Inventory Deleted successfully."});
        }catch(error){
            console.log("error", error);
            setToastInfo({type:"error", message:"Something went wrong."});
        } finally {
    ...
    const getInventory = useCallback(async (pageNumber, searchText) => {
        try {
            setIsLoading(true);
            const response = (await axios.get('http://192.168.1.13:8000/api/inventory',{ params: { page: pageNumber || 1, search: searchText }, ...getHeaderOptions(token) })).data;
            console.log("Response", response);
        ...
        <div className="inventory-page">
            ...
            {showDeleteModal && 
                <DeleteModal 
                    ...
                    handleDelete={() => handleDeleteInventory(showDeleteModal.inventory_id)}  
                />
            }
            <CustomTable
